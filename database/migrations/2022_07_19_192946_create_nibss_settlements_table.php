...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('nibss_settlements', function (Blueprint $table) {
            $table->id();
            ...
            $table->index(['BatchNumber', 'SessionId', 'Amount']);
            $table->index(['Status']);
            $table->index(['BatchNumber', 'Status']);
            $table->index(['BatchNumber', 'RequestDate']);
            $table->index(['Amount', 'SessionId']);
    ...
    {
        Schema::dropIfExists('nibss_settlements');
    }
};
